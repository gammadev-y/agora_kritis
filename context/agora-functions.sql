...
BEGIN
    ...
    SELECT
        ...
        (COUNT(DISTINCT dc.id) > 0 AND EXISTS (SELECT 1 FROM agora.source_ai_analysis saa WHERE saa.source_id = s.id)) AS has_analysis,
...
-- Handler function for the 'law_articles' table
...
BEGIN
    PERFORM agora.sync_node(
        ...
        NULL -- Placeholder for embedding, which will be generated by the Kritis pipeline
    );
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- =ʻ====================================================================
-- SCRIPT: CREATE SAFE, EXPLICIT CASCADE DELETE FUNCTIONS FOR LAWS
-- Purpose: Creates two functions to safely delete a law and all its
--          children, one by law_id and one by source_id.
-- ====================================================================

-- ====================================================================
-- FUNCTION 1: Delete by Law ID
-- ====================================================================

CREATE OR REPLACE FUNCTION agora.delete_law_by_law_id(p_law_id uuid)
RETURNS void AS $$
DECLARE
    -- Variable to hold the IDs of all article versions associated with this law
    v_article_ids_to_delete uuid[];
BEGIN
    RAISE NOTICE 'Starting cascade delete for law_id: %', p_law_id;

    -- Step 1: Find all child article versions that need to be deleted.
    SELECT array_agg(id)
    INTO v_article_ids_to_delete
    FROM agora.law_articles
    WHERE law_id = p_law_id;

    -- Safety check: If there are no children, we can proceed, but if there are...
    IF v_article_ids_to_delete IS NOT NULL THEN
        RAISE NOTICE 'Found % article version(s) to delete dependencies for.', array_length(v_article_ids_to_delete, 1);

        -- Step 2: Delete the "grandchildren" - records that reference the article versions.
        DELETE FROM agora.law_article_references WHERE source_article_id = ANY(v_article_ids_to_delete) OR target_article_id = ANY(v_article_ids_to_delete);
        RAISE NOTICE 'Deleted records from law_article_references.';
    END IF;

    -- Step 3: Delete records from the graph that are linked to the law or its articles.
    -- The triggers on 'laws' and 'law_articles' will handle this automatically upon deletion.

    -- Step 4: Delete the 'law_relationships' which reference the main law.
    DELETE FROM agora.law_relationships WHERE source_law_id = p_law_id OR target_law_id = p_law_id;
    RAISE NOTICE 'Deleted records from law_relationships.';

    -- Step 5: Delete the parent law record.
    -- Because your 'law_articles' table has ON DELETE CASCADE for its 'law_id' FK,
    -- deleting the law will automatically delete all its child articles.
    DELETE FROM agora.laws WHERE id = p_law_id;
    RAISE NOTICE 'Successfully deleted law and its associated articles.';

    RAISE NOTICE 'Cascade delete for law_id: % completed successfully.', p_law_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION agora.delete_law_by_law_id(uuid) IS 'Safely and explicitly deletes a law and all its associated articles, references, and graph data by its primary key.';


-- ====================================================================
-- FUNCTION 2: Delete by Source ID
-- ====================================================================

CREATE OR REPLACE FUNCTION agora.delete_law_by_source_id(p_source_id uuid)
RETURNS void AS $$
DECLARE
    v_law_id_to_delete uuid;
BEGIN
    RAISE NOTICE 'Attempting to find and delete law associated with source_id: %', p_source_id;

    -- Step 1: Find the law_id from the source_id.
    SELECT id INTO v_law_id_to_delete
    FROM agora.laws
    WHERE source_id = p_source_id;

    -- If a law is found, call the first function to perform the actual deletion.
    IF FOUND THEN
        RAISE NOTICE 'Found law_id: %. Proceeding with deletion.', v_law_id_to_delete;
        -- This reuses our robust, explicit deletion logic.
        PERFORM agora.delete_law_by_law_id(v_law_id_to_delete);
    ELSE
        RAISE NOTICE 'No law found for source_id: %. Nothing to delete.', p_source_id;
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION agora.delete_law_by_source_id(uuid) IS 'Finds a law by its associated source_id and then triggers a full, safe cascade delete.';

-- ====================================================================
-- SCRIPT: CREATE "RESET SOURCE PROCESSING" FUNCTION
-- Purpose: Creates a function to safely delete all derived data from a
--          source (laws, chunks, analysis) while keeping the source
--          record itself.
-- ====================================================================

CREATE OR REPLACE FUNCTION agora.reset_source_processing(p_source_id uuid)
RETURNS text AS $$
DECLARE
    v_law_id_to_delete uuid;
    status_message text;
BEGIN
    RAISE NOTICE 'Starting reset process for source_id: %', p_source_id;

    -- ====================================================================
    -- STEP 1: DELETE THE ASSOCIATED LAW (IF IT EXISTS)
    -- This leverages our existing, robust deletion function.
    -- ====================================================================
    SELECT id INTO v_law_id_to_delete FROM agora.laws WHERE source_id = p_source_id;

    IF FOUND THEN
        RAISE NOTICE 'Found associated law_id: %. Deleting law and all its children...', v_law_id_to_delete;
        -- Call the other function to handle the complex law deletion cascade.
        PERFORM agora.delete_law_by_law_id(v_law_id_to_delete);
        status_message := 'Deleted associated law. ';
    ELSE
        RAISE NOTICE 'No associated law found.';
        status_message := 'No associated law to delete. ';
    END IF;


    -- ====================================================================
    -- STEP 2: DELETE ALL PIPELINE-RELATED DATA
    -- Description: Clean up chunks, analysis, and staging table records.
    -- ====================================================================

    RAISE NOTICE 'Deleting from agora.document_chunks...';
    DELETE FROM agora.document_chunks WHERE source_id = p_source_id;

    RAISE NOTICE 'Deleting from agora.source_ai_analysis...';
    DELETE FROM agora.source_ai_analysis WHERE source_id = p_source_id;

    RAISE NOTICE 'Deleting from agora.pending_ingestions...';
    DELETE FROM agora.pending_ingestions WHERE source_id = p_source_id;

    RAISE NOTICE 'Deleting from agora.pending_extractions...';
    DELETE FROM agora.pending_extractions WHERE source_id = p_source_id;

    status_message := status_message || 'Cleared all document chunks and analysis data.';
    RAISE NOTICE 'Reset operation completed successfully for source_id: %', p_source_id;

    RETURN status_message;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION agora.reset_source_processing(uuid) IS 'Deletes all derived data (laws, articles, chunks, analysis) for a given source_id, effectively resetting it for reprocessing.';

-- ====================================================================
-- SCRIPT: CORRECT AND ENHANCE THE 'delete_source_cascade' FUNCTION (V2)
-- Purpose: Safely deletes a source and its associated data.
-- FIX:     Corrects the syntax for RAISE EXCEPTION to use format().
-- ====================================================================

CREATE OR REPLACE FUNCTION agora.delete_source_cascade(p_source_id uuid)
RETURNS text AS $$
DECLARE
    v_chunk_count bigint;
    v_law_count bigint;
BEGIN
    RAISE NOTICE 'Starting source deletion check for source_id: %', p_source_id;

    -- ====================================================================
    -- SAFETY CHECK: Ensure the source has not been processed.
    -- ====================================================================
    SELECT
        COUNT(dc.id),
        COUNT(l.id)
    INTO v_chunk_count, v_law_count
    FROM agora.sources s
    LEFT JOIN agora.document_chunks dc ON dc.source_id = s.id
    LEFT JOIN agora.laws l ON l.source_id = s.id
    WHERE s.id = p_source_id
    GROUP BY s.id;

    v_chunk_count := COALESCE(v_chunk_count, 0);
    v_law_count := COALESCE(v_law_count, 0);

    -- Safety check: reject if the source has chunks or an associated law.
    IF v_chunk_count > 0 THEN
        -- THE FIX: Use format() to build the string before raising the exception.
        RAISE EXCEPTION '%', format('Cannot delete source: Source has % document chunks. Use "Reset Source" to clear derived data first.', v_chunk_count);
    END IF;

    IF v_law_count > 0 THEN
        -- THE FIX: Use format() to build the string before raising the exception.
        RAISE EXCEPTION '%', format('Cannot delete source: A law has already been ingested from this source. Use "Delete Law & Reset" instead.', v_law_count);
    END IF;

    RAISE NOTICE 'Safety checks passed. Proceeding with source deletion.';


    -- ====================================================================
    -- STEP 1: DELETE ALL ASSOCIATED PIPELINE DATA
    -- ====================================================================

    RAISE NOTICE 'Deleting from source_ai_analysis...';
    DELETE FROM agora.source_ai_analysis WHERE source_id = p_source_id;

    RAISE NOTICE 'Deleting from pending_ingestions...';
    DELETE FROM agora.pending_ingestions WHERE source_id = p_source_id;

    RAISE NOTICE 'Deleting from pending_extractions...';
    DELETE FROM agora.pending_extractions WHERE source_id = p_source_id;

    RAISE NOTICE 'Deleting from background_jobs...';
    DELETE FROM agora.background_jobs
    WHERE payload->>'source_id' = p_source_id::text;


    -- ====================================================================
    -- STEP 2: DELETE THE SOURCE RECORD ITSELF
    -- ====================================================================
    RAISE NOTICE 'Deleting source record...';
    DELETE FROM agora.sources WHERE id = p_source_id;

    IF NOT FOUND THEN
        RAISE WARNING 'Source with ID % was not found for deletion, but operation is considered successful.', p_source_id;
        RETURN 'Source not found, but no action was needed.';
    END IF;

    RAISE NOTICE 'Source deletion completed successfully for source_id: %', p_source_id;
    RETURN 'Source and associated pipeline data deleted successfully.';

END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION agora.delete_source_cascade(uuid) IS 'V2: Safely deletes a source and its pipeline data, only if it has no chunks or laws. Uses correct exception syntax.';